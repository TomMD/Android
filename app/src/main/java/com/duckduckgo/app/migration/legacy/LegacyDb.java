...
public class LegacyDb implements Closeable {
  ...
  private LegacyFeedObject getFeedObject(Cursor c) {
    ...
    final String description =
        c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_DESCRIPTION));
    ...
    final String imageurl =
        c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_IMAGE_URL));
    final String favicon =
        c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_FAVICON));
    final String timestamp =
        c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_TIMESTAMP));
    final String category =
        c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_CATEGORY));
    final String type = c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_TYPE));
    final String articleurl =
        c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_ARTICLE_URL));
    final String hidden = c.getString(c.getColumnIndex(LegacyDbContracts.FEED_TABLE.COLUMN_HIDDEN));
    return new LegacyFeedObject(
        id,
        title,
        description,
        feed,
  ...
  public Cursor getCursorSavedSearch() {
    return this.db.query(
        LegacyDbContracts.SAVED_SEARCH_TABLE.TABLE_NAME,
        null,
        null,
        null,
        null,
        null,
        LegacyDbContracts.SAVED_SEARCH_TABLE._ID + " DESC");
  }
  ...
  public boolean isSaved(String id) {
    ...
    try {
      c =
          this.db.query(
              LegacyDbContracts.FEED_TABLE.TABLE_NAME,
              null,
              LegacyDbContracts.FEED_TABLE._ID
                  + "=? AND "
                  + LegacyDbContracts.FEED_TABLE.COLUMN_FAVORITE
                  + "!='F'",
              new String[] {id},
              null,
              null,
  ...
  private static class OpenHelper extends SQLiteOpenHelper {
    ...
    private void createFeedTable(SQLiteDatabase db) {
      ...
      db.execSQL(
          "CREATE INDEX idx_id ON "
              + LegacyDbContracts.FEED_TABLE.TABLE_NAME
              + " ("
              + LegacyDbContracts.FEED_TABLE._ID
              + ") ");
      db.execSQL(
          "CREATE INDEX idx_idtype ON "
              + LegacyDbContracts.FEED_TABLE.TABLE_NAME
              + " ("
              + LegacyDbContracts.FEED_TABLE._ID
              + ", "
    ...
    private void createAppTable(SQLiteDatabase db) {
      ...
      String sql =
          "CREATE VIRTUAL TABLE "
              ...
              + " USING FTS3 ("
              + LegacyDbContracts.APP_TABLE.COLUMN_TITLE
              + " VARCHAR(300), "
              + LegacyDbContracts.APP_TABLE.COLUMN_PACKAGE
              + " VARCHAR(300) "
              + ")";
    ...
    private void createHistoryTable(SQLiteDatabase db) {
      db.execSQL(
          "CREATE TABLE "
              + LegacyDbContracts.HISTORY_TABLE.TABLE_NAME
              + "("
              + LegacyDbContracts.HISTORY_TABLE._ID
              + " INTEGER PRIMARY KEY, "
              + LegacyDbContracts.HISTORY_TABLE.COLUMN_TYPE
              + " VARCHAR(300), "
              + LegacyDbContracts.HISTORY_TABLE.COLUMN_DATA
              + " VARCHAR(300), "
              + LegacyDbContracts.HISTORY_TABLE.COLUMN_URL
              + " VARCHAR(300), "
              + LegacyDbContracts.HISTORY_TABLE.COLUMN_EXTRA_TYPE
              + " VARCHAR(300), "
              + LegacyDbContracts.HISTORY_TABLE.COLUMN_FEED_ID
              + " VARCHAR(300)"
              + ")");
    }
    ...
    private void createSavedSearchTable(SQLiteDatabase db) {
      ...
      String sql =
          "CREATE TABLE "
              ...
              + " INTEGER PRIMARY KEY, "
              + LegacyDbContracts.SAVED_SEARCH_TABLE.COLUMN_TITLE
              + " VARCHAR(300), "
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
      if (oldVersion == 4 && newVersion >= 12) {
        ...
        Cursor c =
            db.query(
                LegacyDbContracts.FEED_TABLE.TABLE_NAME + "_old",
                new String[] {"url"},
                LegacyDbContracts.FEED_TABLE.COLUMN_FEED + "=''",
                null,
                null,
        ...
        db.execSQL(
            "DROP TABLE IF EXISTS " + LegacyDbContracts.SAVED_SEARCH_TABLE.TABLE_NAME + "_old");
      } else {
        dropTables(db);
        onCreate(db);
      }
